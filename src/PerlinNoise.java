...
{
    private final static int DEFAULT_RAND_SEED = 12345;
    ...
    public PerlinNoise() { this(-1,256,2, DEFAULT_RAND_SEED); }

    public PerlinNoise(int repeat, int periodSize, int periodNum, int randSeed) {
        p = new int[periodSize * periodNum];
        ArrayList<Integer> randPerm = permutationGen(periodSize, periodNum, randSeed);
        this.repeat = repeat;
        for (int x = 0; x < (periodSize * periodNum); ++x) {
            p[x] = randPerm.get(x % periodSize); // Randomized permutation
            //p[x] = permutation[x % 256]; // Perlin's set permutation array
        }
    }

    public static ArrayList<Integer> permutationGen(int size, int periods, int rSeed) {
        ArrayList<Integer> unRandValues = new ArrayList<>();
        ArrayList<Integer> randValues = new ArrayList<>();
        Random indexGen = new Random(rSeed);

        // Generate unrandomized permutations
        ...
        int numPerms = unRandValues.size();
        for (int t = 0; t < numPerms; t++) {
            int curIndex = indexGen.nextInt(unRandValues.size());
            randValues.add(unRandValues.get(curIndex));
            unRandValues.remove(curIndex);
        }
